...
class FirebaseUserListener {
    ...
    func downloadUserFromFirebase(userID: String, email: String? = nil){
        FirebaseReference(collectionReference: .User).document(userID).getDocument { (querySnapshot, error) in
    ...
    func downloadAllUsersFromFirebase(completion: @escaping (_ allUsers: [User]) -> Void){
        ...
        FirebaseReference(collectionReference: .User).limit(to: 500).getDocuments { (querySnapshot, error)
            in
    ...
    func updateUserInFirebase(_ user: User) {
        do {
            let _ = try FirebaseReference(collectionReference: .User).document(user.id).setData(from: user)
        } catch {
